var objPrimary = (function(){
    ...
    var eventF = function(){
        ...
        this.triggerEvents = function(){
            ...
            (new ajaxF()).getDataWitnessesById();
            // carga combo city
        ...
        this.txtPuestoVotacion = function(){
            $("#txtPuestoVotacion").devbridgeAutocomplete({
                ...
                onSelect: function (suggestion) {
                    ...
                    $("#txtMesaVotacion").val("").prop("disabled",null);
                },
    ...
    var ajaxF = function(){
        ...
        this.verifyCedulaDB = function(cc){
            ...
            if(cc == $("#hdnNumeroIdentificacion").val()){
                return;
            }
         ...
         * @returns {undefined}
         */
        this.getDataWitnessesById = function(){
            $.sy_ajaxFrm({
                url:$.sy_pathUrl('collaborators/witnesses/read/data/'+$.sy_get_id_token_update()),
                alertError:false,
                success: function(params,response){
                    if(response.msg != null && response.msg.rows != null){
                        // set daatos en form
                        $("#txtId").val(response.msg.rows[0].id).change();
                        $("#txtFechaCreacion").val(response.msg.rows[0].created_at).change();
                        $("#txtNumeroIdentificacion, #hdnNumeroIdentificacion").val(response.msg.rows[0].identification_number).change();
                        
                        // data basic
                        $("#txtPrimerApellido").val(response.msg.rows[0].last_first_name).change();
                        $("#txtSegundoApellido").val(response.msg.rows[0].last_second_name).change();
                        $("#txtPrimerNombre").val(response.msg.rows[0].first_first_name).change();
                        $("#txtSegundoNombre").val(response.msg.rows[0].first_second_name).change();
                        
                        // datos contac y partido
                        $("#numNumeroTelefono").val(response.msg.rows[0].phone_number).change();
                        $("#selPartidoPolitico").val(response.msg.rows[0].political_code).change();
                        $("#selPartidoPolitico").material_select();
                        
                        $("#selDepartamento").val(response.msg.rows[0].department_code).change();
                        $("#selDepartamento").material_select();
                        
                        $("#selCiudad").val(response.msg.rows[0].city_code).change();
                        $("#selCiudad").material_select();
                        
                        // puesto
                        $("#txtPuestoVotacion").val(response.msg.rows[0].place_name).addClass("valid");
                        $("#lblPuestoVotacion").addClass("active");
                        
                        $("#hdnPuestoVotacion").val(response.msg.rows[0].places_id).change();
                        $("#hdnPuestoVotacionMesas").val(response.msg.rows[0].tables_count).change();
                        $("#hdnPuestoVotacionMesasUsed").val(response.msg.rows[0].used_tables).change();
                        
                        $("#txtMesaVotacion").val(response.msg.rows[0].number_table).prop("disabled",null).change();
                        
                        if(response.msg.rows[0].photo_1 != null){
                            $("#aImagen1").attr("href",response.msg.rows[0].photo_1).show();
                        }
                        if(response.msg.rows[0].photo_2 != null){
                            $("#aImagen2").attr("href",response.msg.rows[0].photo_2).show();
                        }
                    }
                }
            }); // END function
